...
const Home = () => {
  ...
  const handleProgressBarPress = event => {
    ...
    const seekTo = (locationX / progressBarWidth) * duration;

    if (videoModalRef.current) {
      videoModalRef.current.seek(seekTo);
    }
  ...
  const handleTouchMove = event => {
    ...
    const {locationX} = event.nativeEvent;
    const seekTo = (locationX / progressBarWidth) * duration;
    const clampedTime = Math.max(0, Math.min(seekTo, duration));
  ...
  const handleTouchEnd = () => {
    setIsDragging(false);
    if (videoModalRef.current) {
      videoModalRef.current.seek(currentTime);
    }
  };
  ...
  // Get original post ID for repost
  const getOriginalPostId = post => {
    // If it's already a repost, get the original post ID from repost_id
    if (post?.repost_id) {
      return post.repost_id;
    }
    // If it's a repost of a repost, get the original post ID from the nested repost
    if (post?.repost?.repost_id) {
      return post.repost.repost_id;
    }
    // If it's an original post, return its own ID
    return post?.id;
  };
  ...
  const hasUserReposted = post => {
    ...
    // Check if any post in allPosts is a repost by current user of this original post
    return allPosts.some(
      p =>
        p.parent_id === userData.id &&
        (p.repost_id === originalPostId ||
          (p.repost && p.repost.id === originalPostId)),
    );
  };
  ...
  const fetchPosts = async (requestedPage = 1, replace = false) => {
    try {
      ...
      const response = await axios.get(
        `https://beingpetz.com/petz-info/public/api/v1/post/all`,
        {
  ...
  const handleLikePost = async postId => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://beingpetz.com/petz-info/public/api/v1/post/like',
        formData,
        {
  ...
  const handleRepost = async (item, fetchPostsCallback) => {
    try {
      ...
      // Check if user has already reposted this post
      if (hasUserReposted(item)) {
        Dialog.show({
          type: ALERT_TYPE.WARNING,
          title: 'Already Reposted',
          textBody: 'You have already reposted this post.',
          button: 'OK',
        });
        return;
      }
      ...
      // Always send the original post ID, not the repost ID
      const originalPostId = getOriginalPostId(item);
      formData.append('post_id', originalPostId.toString());

      formData.append('is_public', '1');

      const response = await axios.post(
        'https://beingpetz.com/petz-info/public/api/v1/post/re-post',
        formData,
        {
          headers: {'Content-Type': 'multipart/form-data'},
          timeout: 10000,
      ...
      if (response.data.status === true) {
        ...
        throw new Error(response.data.message || 'Repost failed');
      }
    } catch (error) {
      ...
      if (error.response) {
        ...
        if (responseData.errors) {
          ...
          errorMessage =
            responseData.errors[errorFields[0]]?.[0] ||
            responseData.message ||
            'Validation error';
        } else {
          errorMessage =
            responseData.message || error.response.statusText || 'Server error';
        }
    ...
    return (
      <View style={styles.birthdayContainer}>
        <View style={styles.birthdayHeader}>
          <Image
            source={{
              uri: `https://beingpetz.com/petz-info/public/${item?.parent?.profile}`,
            }}
        ...
        <View style={styles.birthdayContent}>
          ...
          <View style={styles.petInfo}>
            <Image
              source={{
                uri: `https://beingpetz.com/petz-info/public/${item?.pet?.avatar}`,
              }}
  ...
  const PostCardOriginal = ({item}) => {
    ...
    const renderMediaItem = (media, index) => {
      const uri = `https://beingpetz.com/petz-info/public/${media.uri}`;
      return (
        <TouchableOpacity
          key={index}
      ...
      return (
        <TouchableOpacity
          ...
          {media.isVideo ? (
            ...
            <Image
              source={{uri}}
              style={{width: '100%', height: 400, borderRadius: 8,marginLeft:8}}
              resizeMode='cover'
            />
          )}
        </TouchableOpacity>
  ...
  const PostActions = ({item}) => {
    const hasReposted = hasUserReposted(item);
  ...
  const fetchComments = async postId => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://beingpetz.com/petz-info/public/api/v1/post/get-comment',
        formData,
        {
          headers: {'Content-Type': 'multipart/form-data'},
        },
  ...
  const handlePostComment = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://beingpetz.com/petz-info/public/api/v1/post/comment',
        formData,
        {
          headers: {'Content-Type': 'multipart/form-data'},
        },
  ...
  const fetchFriendSuggestions = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://beingpetz.com/petz-info/public/api/v1/pet/friends/suggestions',
        formData,
        {
          headers: {'Content-Type': 'multipart/form-data'},
        },
  ...
  const fetchMyPosts = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://beingpetz.com/petz-info/public/api/v1/post/get/my',
        formData,
        {
          headers: {'Content-Type': 'multipart/form-data'},
        },
  ...
  const sendFriendRequest = async to_parent_id => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://beingpetz.com/petz-info/public/api/v1/pet/friends/send-request',
        formData,
        {
          headers: {'Content-Type': 'multipart/form-data'},
        },
        ...
        {text: 'Cancel', style: 'cancel'},
        {
          ...
          onPress: async () => {
            try {
              ...
              const response = await axios.post(
                'https://beingpetz.com/petz-info/public/api/v1/post/delete',
                formData,
                {
                  headers: {'Content-Type': 'multipart/form-data'},
                },
          ...
          {text: 'Cancel', style: 'cancel'},
          {
            ...
            onPress: async () => {
              try {
                ...
                const response = await axios.post(
                  'https://beingpetz.com/petz-info/public/api/v1/hide-post',
                  formData,
                  {
                    headers: {'Content-Type': 'multipart/form-data'},
                  },
  ...
  const handleReport = async (post, reason) => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://beingpetz.com/petz-info/public/api/v1/report/add',
      ...
      <FlatList
        ...
        renderItem={({item: sItem}) => (
          <View style={styles.suggestFriendcard}>
            <Image
              source={{
                uri: `https://beingpetz.com/petz-info/public/${sItem?.profile}`,
              }}
  ...
  return (
    <AlertNotificationRoot>
      <View style={styles.container}>
        {/* <StatusBar backgroundColor="#3B0060" barStyle="dark-content" /> */}
        <FlatList
        ...
        {/* Video Modal with Progress Bar */}
        <Modal
          visible={!!modalVideoUri}
          transparent={true}
          statusBarTranslucent={true}
              ...
              {modalVideoUri && (
                <>
                  <Video
                    ...
                    style={{width: '100%', height: '100%'}}
                    resizeMode="contain"
                    paused={modalVideoPaused}
                    repeat
                    onProgress={handleVideoProgress}
                    onLoad={handleVideoLoad}
                    onEnd={() => setModalVideoPaused(true)}
                  />

                  {/* Play/Pause Overlay */}
                  <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={() => setModalVideoPaused(!modalVideoPaused)}
                    style={{
                      position: 'absolute',
                      top: 0,
                      left: 0,
                      ...
                      alignItems: 'center',
                      backgroundColor: modalVideoPaused
                        ? 'rgba(0,0,0,0.4)'
                        : 'transparent',
                    }}>
                    {modalVideoPaused && (
                  ...
                  <View
                    ...
                    <Text style={{color: 'white', fontSize: 12, width: 40}}>
          ...
          <Animated.View
            ...
            <View style={styles.sheetHeader}>
              <View style={styles.sheetPostInfo}>
                <Image
                  source={{
                    uri: `https://beingpetz.com/petz-info/public/${selectedPostForComment?.parent?.profile}`,
                  }}
                  style={styles.smallProfileImage}
                  defaultSource={require('../Assests/Images/dog.png')}
                />
                <Text style={styles.postUser}>
                  {selectedPostForComment?.parent?.first_name
...
const styles = StyleSheet.create({
  ...
  postCaption: {color: '#333', fontSize: 14, paddingLeft: 10, lineHeight: 18},
