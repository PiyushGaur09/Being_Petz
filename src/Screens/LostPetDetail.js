...
const PetDetails = ({route}) => {
  ...
  // Calculate age
  const age =
    pet?.dob || pet?.pet_dob ? calculateAge(pet.dob || pet.pet_dob) : 'Age not available';
        ...
        if (petImage && petImage.uri) {
          ...
          Image.getSize(
            ...
            (w, h) => {
              ...
              if (computedHeight > MAX_IMAGE_HEIGHT) computedHeight = MAX_IMAGE_HEIGHT;
          ...
          if (resolved && resolved.width && resolved.height) {
            ...
            let computedHeight = Math.max(60, Math.round(resolved.height * scale));
            if (computedHeight > MAX_IMAGE_HEIGHT) computedHeight = MAX_IMAGE_HEIGHT;
            setDisplaySize({width: DISPLAYED_WIDTH, height: computedHeight});
          }
        }
        ...
        <View style={styles.contentContainer}>
          <View style={styles.headerSection}>
            <View style={styles.metaContainer}>
              <View style={styles.metaRow}>
                <View style={styles.metaItem}>
                  <Icon name={getGenderIcon(gender)} size={18} color={'#8337B2'} />
                  <Text style={styles.metaText}>{capitalizeFirstLetter(gender)}</Text>
                </View>
              ...
              {breed && (
                <View style={styles.metaItem}>
                  <Icon name={'dog'} size={18} color={'#8337B2'} />
                  <Text style={styles.metaText}>{capitalizeFirstLetter(breed)}</Text>
                </View>
              )}

              <View style={styles.metaItem}>
                <Icon name="map-marker" size={18} color="#8337B2" />
                <Text style={styles.metaText}>{capitalizeFirstLetter(location)}</Text>
              </View>
            </View>
          </View>
            ...
            <View style={styles.healthContainer}>
              ...
              <View style={styles.healthGrid}>
                {healthIndicators.map(indicator => (
                  <View
                    key={indicator.id}
                    style={[styles.healthItem, {backgroundColor: indicator?.backgroundColor}]}>
                    <View style={styles.healthIconContainer}>
                      <Icon name={indicator.icon} size={28} color={'#fff'} />
                    </View>
                    <Text style={styles.healthLabel}>{indicator.name}</Text>
          ...
          <View style={styles.aboutContainer}>
            <Text style={styles.sectionTitle}>
              {isLostPet ? 'How to identify your pet?' : isFoundPet ? 'Any identification?' : `About ${petName}`}
            </Text>
            <Text style={styles.aboutText}>{aboutPet}</Text>
          </View>
