...
//   const [email_phone, setemail_phone] = useState('devp7536@gmail.com');
...
//           webClientId:
//             '479650253156-kcvfuq359p0hm6t8mcfirprasvg6hag7.apps.googleusercontent.com', // ✅ Correct Web client ID
...
//       const response = await axios.post(
//         'https://argosmob.uk/being-petz/public/api/v1/auth/login',
//         formData,
//         {
//           headers: {
//             'Content-Type': 'multipart/form-data',
...
//         if (userData?.id) {
//           await fetchUserDetail(userData.id);
//         } else {
//           setIsLoading(false);
//           console.warn('User ID is missing, cannot fetch details.');
//         }
//       } else {
//         setIsLoading(false);
//         Alert.alert(
//           'Login Failed',
//           'Invalid credentials or missing user data.',
//         );
//       }
//     } catch (error) {
//       setIsLoading(false);
...
//                 style={[styles.signupLink, {marginLeft: 5}]}>
//                 Read Now
//               </Text>
//             </View>
...
const Login = () => {
  ...
  useEffect(() => {
    GoogleSignin.configure({
      webClientId: '479650253156-kcvfuq359p0hm6t8mcfirprasvg6hag7.apps.googleusercontent.com',
      offlineAccess: true,
    });
  }, []);
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      const response = await axios.post(
        'https://argosmob.uk/being-petz/public/api/v1/auth/google-login',
        { token: idToken }
      );
  ...
  const fetchUserDetail = async userId => {
    try {
      ...
      navigation.navigate(user?.pets?.length > 0 ? 'BottomNavigation' : 'Pet Form');
    } catch (error) {
      console.error('Failed to fetch user detail:', error);
      Alert.alert('Error', 'Something went wrong. Please try again.');
    }
          ...
          <View style={styles.formContainer}>
            ...
            <View style={styles.socialIcons}>
              <TouchableOpacity 
                ...
                />
                {googleLoading && (
                  <ActivityIndicator style={styles.socialLoader} color="#8337B2" />
                )}
              </TouchableOpacity>
              <Image source={require('../Assests/Images/facebookLogo.png')} />
              <Image source={require('../Assests/Images/appleLogo.png')} />
