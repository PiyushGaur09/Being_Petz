...
const CreatePost = () => {
  ...
  // Add this permission request function
  ...
  // Request audio permission for video recording (Android)
    ...
    try {
      ...
      setMedia(prev => [
        ...prev,
        {
          uri: image.path,
          type: image.mime,
          name: `photo_${Date.now()}.${image.path.split('.').pop()}`,
          isVideo: false,
        },
      ]);
    } catch (error) {
    ...
    try {
      ...
      if (video.duration > 30000) {
        Toast.show({
          ...
          textBody: 'Please record a video of max 30 seconds.',
        });
        return;
      }
  ...
  //     const tooLong = selectedArray.some(v => v.duration > 30000);
  //     console.log('too long ', selectedArray);
  //     if (tooLong) {
  ...
  //       // //   type: ALERT_TYPE.DANGER,
  //       // //   title: 'Video too long',
  //       // //   textBody: 'Please select a video of max 30 seconds.',
  //       // // });
  //       return;
  //     }
  ...
  const handleAddVideo = async () => {
    ...
    if (!hasPermission) {
      Alert.alert(
        'Permission Denied',
        'Cannot select video without storage permission.',
      );
      return;
    }
    ...
    try {
      ...
      setMedia(prev => [
        ...prev,
        {
          ...
          name: `photo_${Date.now()}.${croppedImage.path.split('.').pop()}`,
          isVideo: false,
        },
      ]);
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      // Separate featured media from other media
      const featuredImage = media.find(item => !item.isVideo);
      const featuredVideo = media.find(item => item.isVideo);

      // All other images (excluding featured image if exists)
      const otherImages = media.filter(
        (item, index) =>
          !item.isVideo && (!featuredImage || item.uri !== featuredImage.uri),
      );

      // All other videos (excluding featured video if exists)
      const otherVideos = media.filter(
        (item, index) =>
          item.isVideo && (!featuredVideo || item.uri !== featuredVideo.uri),
      );

      // Add featured media if they exist
      if (featuredImage) {
      ...
      // Add other images with proper array indexing
      ...
      // Add other videos with proper array indexing
      ...
      const response = await axios.post(
        'https://argosmob.uk/being-petz/public/api/v1/post/create',
        formData,
        {
          headers: {
...
//   const [currentCropIndex, setCurrentCropIndex] = useState(0);
//   const [isCropping, setIsCropping] = useState(false);
//   const listenerSubscription = useRef({});
...
//     listenerSubscription.current.onError = eventEmitter.addListener(
//       'onError',
//       ({message}) => {
//         Toast.show({
//           type: ALERT_TYPE.DANGER,
//           title: 'Error',
//           textBody: 'Failed to trim video: ' + message,
//         });
//       },
//     );
...
//             PermissionsAndroid.PERMISSIONS.READ_MEDIA_VIDEO,
//           );
//           return readMediaVideo === PermissionsAndroid.RESULTS.GRANTED;
//         } else {
//           const granted = await PermissionsAndroid.request(
//             PermissionsAndroid.PERMISSIONS.READ_EXTERNAL_STORAGE,
//             {
...
//       //   type: ALERT_TYPE.WARNING,
//       //   title: 'Permission required',
//       //   textBody: 'Storage permission is needed to select videos',
//       // });
...
//     if (!videoUri) return;
//     const isFileValid = await isValidFile(videoUri);

//     if (!isFileValid) {
//       Alert.alert('Invalid Video');
//       return;
//     }
...
//   //   setError('Failed to select video: ' + error.message);
//   //   Toast.show({
//   //     type: ALERT_TYPE.DANGER,
//   //     title: 'Error',
//   //     textBody: 'Failed to select video: ' + error.message,
//   //   });
//   // }
//   // };
