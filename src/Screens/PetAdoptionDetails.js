...
import Icon from 'react-native-vector-icons/MaterialCommunityIcons';
import {useNavigation} from '@react-navigation/native';
import CommonHeader from '../Screens/Components/CommonHeader';
import FriendRequestsModal from './Components/FriendRequestsModal';
...
const PetAdoptionDetails = ({route = {params: {}}}) => {
  ...
  // Calculate age helper (years + months)
  function calculateAge(dob) {
    if (!dob) return 'Age not available';
    const birthDate = new Date(dob);
    const today = new Date();
  ...
  // Share handler using your exact message template
  const handleShare = async item => {
    try {
      if (!item) return;
      ...
      const ageMonths =
        monthsSinceDob !== null ? `${monthsSinceDob} months` : 'Unknown';

      const message = `
Adopt Pet Alert 🐾

Breed: ${item.breed || 'Unknown Breed'}
Description: ${item.description || item.about_pet || 'No description provided'}
...
Adopt or share via our app: https://play.google.com/store/apps/details?id=com.being_petz&pcampaignid=web_share
      ...
      console.error('Error sharing pet info:', error);
      Alert.alert('Error', 'Could not share pet info. Please try again.');
    }
  };
  ...
  const healthIndicators = [
    {
      id: 1,
      name: 'Dewormed',
      status: pet?.is_dewormed ?? pet?.dewormed ?? pet?.is_healthy ?? false,
      icon: 'pill',
      backgroundColor: '#c486eb',
    },
    {
      ...
      if (petImage && petImage.uri) {
        ...
        Image.getSize(
          ...
          (w, h) => {
            ...
            const computedHeight = Math.max(
              60,
              Math.round((h || DISPLAYED_WIDTH) * scale),
            ); // min height 60
            setDisplaySize({width: DISPLAYED_WIDTH, height: computedHeight});
          },
          error => {
            if (!isMounted) return;
            console.warn(
              'Image.getSize failed, falling back to default size',
              error,
            );
            // fallback: keep default display size already set
          },
        ...
        try {
          ...
          if (source && source.width && source.height) {
            ...
            const computedHeight = Math.max(
              60,
              Math.round((source.height || DISPLAYED_WIDTH) * scale),
            );
            setDisplaySize({width: DISPLAYED_WIDTH, height: computedHeight});
          } else {
            // fallback: nothing to do
