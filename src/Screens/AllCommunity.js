...
//     try {
//       const response = await axios.get(ALL_COMMUNITIES_API);
//       // Log raw response to help you inspect the exact field names
//       console.log('ALL_COMMUNITIES_API response:', response?.data);
...
//   const fetchMyCommunities = async userId => {
//     try {
//       const response = await axios.post(MY_COMMUNITIES_API, {
//         parent_id: userId,
//       });
//       // log for inspection
//       console.log('MY_COMMUNITIES_API response:', response?.data);
//       if (response.data?.status) {
//         // Some responses wrap inside data.data (as your original code expected)
//         // Ensure defensive chaining
...
//       // Transform communities data; prefer members_count, fallback to members.length
//       const transformedAllCommunities = allCommunitiesData.map(community => {
//         const membersCountFromField =
//           community.members_count ?? // prefer numeric field
//           (typeof community.members_count === 'string'
//             ? Number(community.members_count)
//             : undefined);

//         const membersCountFromArray = Array.isArray(community.members)
//           ? community.members.length
//           : undefined;

//         const finalMembersCount =
//           membersCountFromField ?? membersCountFromArray ?? 0;
...
//           // keep original raw object in case you need it later
//           _raw: community,
//         };
//       });

//       // myCommunitiesData may contain objects with community nested; extract ids defensively
//       const transformedMyCommunities = (myCommunitiesData || [])
//         .map(item => {
//           if (!item) return null;
//           if (item.community && item.community.id) return item.community.id;
//           // some apis return the community id directly
...
//         // 2) increment members_count for the joined community
...
//         // Optionally re-fetch to ensure server-side consistency:
//         // loadCommunities();
//       } else {
//         Alert.alert(
//           'Failed',
...
//   // If screen was opened via deep link, route.params will contain id/name
//   const incomingId = route?.params?.id ?? route?.params?.communityId;
//   const incomingName = route?.params?.name ?? route?.params?.communityName;

//   useEffect(() => {
//     // If we have a name param, set it immediately to prefill search
//     if (incomingName) {
//       setSearchQuery(incomingName);
//     }
//     // If incomingId is present but no name, we can later scroll to it once data loads.
...
//   const loadCommunities = async (scrollToIncoming = true) => {
//     try {
//       setLoading(true);
//       const user = await fetchUserData();
//       if (!user?.id) return;
...
//       // if we have an incoming id and we should scroll, attempt to scroll after short delay
//       if (scrollToIncoming && incomingId) {
//         // give FlatList time to render
//         setTimeout(() => {
//           const index = transformedAllCommunities.findIndex(
...
//               index,
//               animated: true,
//               viewPosition: 0.2, // position the item not exactly at top
//             });
//           }
//         }, 400);
//       }
...
//         );
//       } else {
//         Alert.alert('Failed', response.data.message || 'Could not join community.');
//       }
//     } catch (error) {
//       console.error('Join community error:', error);
//       if (error.response) {
...
//   useFocusEffect(
//     useCallback(() => {
//       // if the screen is reopened via navigation, reload and scroll if needed
//       loadCommunities(true);
//     }, [incomingId, incomingName]),
//   );

//   useEffect(() => {
//     // On first mount also load communities
//     loadCommunities(true);
...
//           // fall back: try again after short delay
//           setTimeout(() => {
//             if (!incomingId) return;
//             const idx = allCommunities.findIndex(
//               c => String(c.id) === String(incomingId),
//             );
